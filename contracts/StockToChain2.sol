...
contract StockToChain2 is ERC20, Ownable, Pausable, ReentrancyGuard {
    ...
    event ProfitShareUpdated(address indexed investor, uint256 amount);
    event PriceFeedError(string feed, int256 price);
    ...
    modifier validPriceFeed(int256 price) {
    ...
    function distributeProfits() external payable onlyOwner whenNotPaused {
        ...
        // Update total distributed profits and investor shares
        ...
        if (totalTokens > 0) {
            for (uint256 i = 0; i < investorList.length; i++) {
                ...
                if (investors[investor].totalTokens > 0) {
                    uint256 share = (distribution.tokenHolderShare * investors[investor].totalTokens) / totalTokens;
                    investors[investor].unclaimedProfits += share;
                    emit ProfitShareUpdated(investor, share);
                }
            }
        }
    ...
    // View Functions
    function getCurrentPriceInPOL(uint256 amount) public view validAmount(amount) returns (uint256) {
        ...
        // Check for overflow
        uint256 numerator = priceInEur * uint256(eurUsdPrice) * 10**18;
        uint256 denominator = uint256(polUsdPrice) * 10**PRICE_FEED_DECIMALS;
        
        require(numerator > priceInEur && numerator > uint256(eurUsdPrice), "Multiplication overflow");
    ...
    // Add partial buyback function
    function partialBuyback(uint256 amount) external payable onlyOwner whenNotPaused {
        require(msg.value > 0, "No funds for buyback");
        require(workflowStatus == WorkflowStatus.BuybackActive, "Buyback not active");
        require(amount > 0 && amount <= totalSupply(), "Invalid amount");
        
        uint256 buybackPrice = getBuybackPriceInPOL(amount);
        require(msg.value >= buybackPrice, "Insufficient funds for buyback");
        ...
        for (uint256 i = 0; i < investorList.length; i++) {
            address investor = investorList[i];
            if (investors[investor].totalTokens > 0) {
                uint256 investorShare = (investors[investor].totalTokens * remainingFunds) / totalTokens;
                if (investorShare > 0) {
                    payable(investor).transfer(investorShare);
                }
            }
